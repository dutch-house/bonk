...
import type { AuctionMarket, AuctionToken, ReentrantBidder } from "../types";

describe("AuctionMarket", () => {
  let auction: AuctionMarket;
  let token: AuctionToken;
  let reBidder: ReentrantBidder;
  ...
  beforeEach(async () => {
    ...
    // Deploy the ReentrantBidder contract
    const auctionAddress = await auction.getAddress()
    const ReentrantBidder = await hre.ethers.getContractFactory("ReentrantBidder");
    reBidder = await ReentrantBidder.deploy(
      auctionAddress
    );
    // Wait for the deployment to be mined
    await reBidder.waitForDeployment();
  });
